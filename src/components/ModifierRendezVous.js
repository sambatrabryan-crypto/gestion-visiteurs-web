...
function ModifierRendezVous({ onBack, rendezvous, onUpdate, apiCall, showAlert }) {
    ...
    // 🔹 Charger les règles de ciblage depuis l'API avec apiCall
    useEffect(() => {
        chargerCiblages();
    }, [apiCall]);

    const initialiserDonneesRendezVous = useCallback(() => {
        if (!rendezvous) return;

        console.log('Données du rendez-vous reçues:', rendezvous);

        // Récupérer le ciblage complet à partir de l'ID ciblage
        const ciblageComplet = ciblagesDisponibles.find(c => c.id_ciblage === rendezvous.id_ciblage);

        if (ciblageComplet) {
            console.log('Ciblage trouvé:', ciblageComplet);
            setFormData({
                nom_visiteur: rendezvous.nom_visiteur || '',
                prenom_visiteur: rendezvous.prenom_visiteur || '',
                telephone: rendezvous.telephone || '',
                email: rendezvous.email || '',
                nom_organisation: rendezvous.nom_organisation || '',
                type_organisation: rendezvous.type_organisation || '',
                id_ciblage: rendezvous.id_ciblage?.toString() || '',
                id_type_visite: ciblageComplet.id_type_visite?.toString() || '',
                id_type_service: ciblageComplet.id_type_service?.toString() || '',
                date_rendezvous: rendezvous.date_rendezvous ? rendezvous.date_rendezvous.slice(0, 16) : '',
                duree_estimee: rendezvous.duree_estimee || 60,
                motif: rendezvous.motif || ''
            });

            setCiblageSelectionne(ciblageComplet);
        } else {
            console.log('Aucun ciblage trouvé pour ID:', rendezvous.id_ciblage);
    ...
    }, [rendezvous, ciblagesDisponibles]);

    // 🔹 Initialiser les données du rendez-vous une fois les ciblages chargés
    useEffect(() => {
        if (rendezvous && ciblagesDisponibles.length > 0) {
            initialiserDonneesRendezVous();
            setInitialLoading(false);
        }
    }, [rendezvous, ciblagesDisponibles, initialiserDonneesRendezVous]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!formData.date_rendezvous) {
                const errorMessage = 'Veuillez sélectionner une date et heure pour le rendez-vous';
                showAlert(errorMessage, "error");
                setLoading(false);
                return;
            }
            ...
            if (!formData.motif.trim()) {
                const errorMessage = 'Veuillez saisir le motif du rendez-vous';
                showAlert(errorMessage, "error");
                setLoading(false);
                return;
            }

            // ✅ CRÉER dataToSend SANS LES CHAMPS INTERDITS
            const dataToSend = {
                ...
                duree_estimee: parseInt(formData.duree_estimee) || 60,
                motif: formData.motif
            };

            console.log('📤 Données envoyées (RDV):', dataToSend);

            const result = await apiCall(`/rendezvous/${rendezvous.id_rendezvous}`, {
                method: 'PUT',
            ...
            const successMessage = result.message || 'Rendez-vous modifié avec succès!';
            ...
            // ✅ Redirection immédiate vers la liste des rendez-vous
            setTimeout(() => {
                onBack();
            ...
            if (error.status === 403 || errorMessage.includes('En cours')) {
                errorMessage = 'Impossible de modifier un rendez-vous en cours de prise en charge';
    ...
    const handleCancel = () => {
        // ✅ Retour direct à la liste des rendez-vous
        onBack();
    };
            ...
            <div className="container mx-auto px-4">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="text-center mb-12">
                        ...
                        {/* ✅ MASQUER L'ID DU RENDEZ-VOUS */}
                        <p className="text-gray-600 text-lg">Modification du rendez-vous</p>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl border border-orange-100 overflow-hidden transform hover:shadow-2xl transition-all duration-300">
                        ...
                        <div className="bg-gradient-to-r from-orange-500 to-red-600 p-6">
                            <div className="flex items-center justify-between">
                                <h2 className="text-2xl font-bold text-white">Modification du Rendez-vous</h2>
                                        ...
                                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border-2 border-blue-100">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={handleInputChange}
                                                        className="w-full px-5 py-4 bg-white border-2 border-blue-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300"
                                                        required
                                                    />
                                                </div>
                                            ...
                                            <div className="mt-6 space-y-3">
                                                ...
                                                <textarea
                                                    ...
                                                    onChange={handleInputChange}
                                                    rows="3"
                                                    className="w-full px-5 py-4 bg-white border-2 border-blue-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 resize-none"
                    ...
                    <div className="text-center mt-8 text-gray-500 text-sm">
                        <p>Système de gestion des rendez-vous</p>
                    </div>
                </div>
            </div>
        </div>
